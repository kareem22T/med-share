    ...
    public function register(Request $request) {
        ...
        if ($user) :
            ...
            return $this->handleResponse(
                ...
                [],
                [
                    "user" => $user->only("name", "email", "phone", "is_email_verified", "is_phone_verified", "lat", "lng", "isApproved"),
                    "token" => $token
                ],
                []
            );
        ...
        if ($user) {
            if (Auth::attempt(['email' => $request->email, 'password' => $request->password])) {
                ...
                return $this->handleResponse(
                    ...
                    [],
                    [
                        "user" => $userManual->only("name", "email", "phone", "user_type", "picture", "is_email_verified", "lat", "lng", "isApproved"),
                        "token" => $token
                    ],
                    [
                    ]
        ...
        if ($user) {
            ...
            if ($user) {
                return $this->handleResponse(
                    ...
                    [],
                    [
                        "user" => $user->only("name", "email", "phone", "user_type", "picture", "is_email_verified", "lat", "lng", "isApproved")
                    ],
                    [
                        "validation" => [
                            "phone" => "متبعتش الرقم في التعديل غير لو اتغير لاني مش هيعدي لو نفس القديم",
    ...
    public function getUser(Request $request) {
        ...
        if ($user) {
            return $this->handleResponse(
                ...
                [],
                [
                    "user" => $user->only("name", "pharmacy_name", "email", "phone", "user_type", "picture", "is_email_verified", "lat", "lng", "isApproved")
                ],
                [
                ]
            );
